...
lunch between 12:00 and 13:00, and dinner between 18:00 and 19:00. Wouldn’t it be nice if you had a 
...
def convert(time):
    ...
    convertTime= time.split(":")
    
    
    if float(convertTime[1]) > 0:
        convertTime[1]=float(convertTime[1])/60
    
    a= float(convertTime[0])
    b = float(convertTime[1])
    
    return a+b


def get_time():
    return input("Write what time is it with this format (#:## or ##:##):" )
